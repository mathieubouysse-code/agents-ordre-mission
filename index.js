...
/* ══════════════════════════════════════════════════════
   Contexte de l'application à analyser.
   Tu peux remplacer ce texte par n'importe quelle
   description d'application que tu veux faire analyser.
   ══════════════════════════════════════════════════════ */
const CONTEXTE_APP = `
...
4. 500+ lignes de handlers dans le composant App racine sans séparation
5. 200 lignes de CSS injecté en milieu de composant
6. ThreadPanel : détection "allCategoriesChosen" basée sur cap1 hardcodé
7. Suppression d'utilisateur ne vérifie pas les brouillons actifs
`;

/* ══════════════════════════════════════════════════════
   Rendu du résultat selon l'agent actif
   ══════════════════════════════════════════════════════ */
...
export default function App() {
  ...
  // Quand le pipeline finit, afficher le comparateur en premier
  useEffect(() => {
    if (phase === "result" && !activeTab) {
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%)", padding: "28px 32px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ fontSize: 11, letterSpacing: "0.15em", color: "#93c5fd", textTransform: "uppercase", marginBottom: 6 }}>
            Analyse multi-agents
          </div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "white" }}>
            Réécriture de{" "}
            <code style={{ background: "rgba(255,255,255,0.1)", padding: "2px 8px", borderRadius: 4, fontSize: 18 }}>
              ordre-mission.jsx
            </code>
          </h1>
          <p style={{ color: "#94a3b8", margin: "8px 0 0", fontSize: 13 }}>
            5 agents analysent le code, identifient les bugs et proposent une version améliorée
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {phase === "accueil" && (
          <div style={{ background: "white", borderRadius: 16, padding: 40, border: "1px solid #e2e8f0", textAlign: "center" }}>
            ...
            <h2 style={{ margin: "0 0 8px", fontSize: 20, color: "#0f172a" }}>
              Analyse de l'application réelle
            </h2>
            <p style={{ color: "#6b7280", fontSize: 14, maxWidth: 500, margin: "0 auto 8px" }}>
              Les 5 agents vont analyser <strong>ordre-mission.jsx</strong> et identifier ce qui aurait changé si on avait utilisé cette méthode dès le départ.
            </p>
            <p style={{ color: "#9ca3af", fontSize: 12, marginBottom: 28 }}>
              🔍 Auditeur → 📐 Architecte → 🔧 Correcteur → 🧪 Testeur → ⚖️ Comparateur
            </p>
            <button
              onClick={() => launch(CONTEXTE_APP)}
              style={{ background: "linear-gradient(135deg, #0f172a, #1e40af)", color: "white", border: "none", borderRadius: 12, padding: "14px 36px", fontWeight: 800, fontSize: 15, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 8px 24px rgba(15,23,42,0.3)" }}
            >
              🚀 Lancer l'analyse
            </button>
          </div>
        ...
        {/* ── Agent en cours ── */}
        {phase === "building" && currentAgent && (
          <LoadingCard agentId={currentAgent} />
        )}
        ...
        {error && (
          <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: 20, marginBottom: 16, color: "#dc2626", fontSize: 14 }}>
            ...
            <span style={{ fontSize: 12, color: "#9f1239" }}>
              Vérifiez que la variable REACT_APP_ANTHROPIC_KEY est bien configurée dans Vercel.
            </span>
          </div>
        )}
        ...
        {Object.keys(results).length > 0 && (
          <div>
            <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
              {STEPS.filter(s => results[s]).map(s => {
                ...
                return (
                  <button
                    ...
                    style={{ background: activeTab === s ? a.color : "white", color: activeTab === s ? "white" : "#374151", border: `1.5px solid ${activeTab === s ? a.color : "#e2e8f0"}`, borderRadius: 10, padding: "8px 16px", fontWeight: 600, fontSize: 12, cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s", display: "flex", alignItems: "center", gap: 6 }}
                  >
              ...
              {phase === "result" && (
                <button
                  onClick={() => { reset(); setActiveTab(null); }}
                  style={{ background: "transparent", color: "#9ca3af", border: "1px solid #e2e8f0", borderRadius: 10, padding: "8px 16px", fontSize: 12, cursor: "pointer", fontFamily: "inherit", marginLeft: "auto" }}
                >
                  ↺ Recommencer
                </button>
              )}
            ...
            {activeTab && results[activeTab] && (
              <AgentCard agentId={activeTab}>
                {renderAgentResult(activeTab, results[activeTab])}
              </AgentCard>
            )}
          </div>
        )}
